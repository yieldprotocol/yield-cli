...
const VAT_ABI = [
    "function urns(bytes32 ilk, address urn) view returns (uint256, uint256)",
    "function ilks(bytes32 ilk) view returns (uint256, uint256, uint256, uint256, uint256)",
]
...
;(async () => {
  ...
  const eth = (await vat.urns(ETH, treasury.address)).ink
  console.log(eth)
  const spot = (await vat.ilks(ETH)).spot
